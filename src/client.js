/**
 * DSH Fleet Cleaner Client Module.
 * Conforms to DSH window.__ModuleLoader__.load({ id, factory }) specification.
 * Integrates cleanly into DSH BetterSidebar Tab without invalid slot registrations.
 * Pure script without raw ESM import/export statements for reliable browser combo loading.
 * UI Version: v0.2.0 (Phase 1 & 2 - Anti-Slop & Bilingual Edition)
 */

(function () {
  function createModule(React) {
    ...
    return { FleetCleanerHud, FleetCleanerTab };
  }

  // --- Client Plugin apply and factory ---
  function apply(ctx, options = {}) {
    const React = options.React || (typeof window !== 'undefined' ? window.React : null) || (ctx?.get ? ctx.get('react') : null);
    if (!React || typeof React.createElement !== 'function') {
      throw new TypeError('dsh-fleet-cleaner [client]: React with createElement is required for client tab registration.');
    }

    const { FleetCleanerTab } = createModule(React);
      ...
      if (ctx?.betterSidebar && typeof ctx.betterSidebar.registerTab === 'function') {
        return ctx.betterSidebar.registerTab({
          ...
          icon: (size) => React.createElement('svg', {
            ...
            React.createElement('path', { key: 'p1', d: 'M5 3h14l2 2v14l-2 2H5l-2-2V5Z' }),
            React.createElement('path', { key: 'p2', d: 'm7 17 10-10m-5 0h5v5M7 11l3-3m3 9 3-3' })
          ]),
          order: 6,
          single: true,
          component: FleetCleanerTab,
        });
      }
      return () => {};
    };
  ...
  function createClientFactory() {
    return (require) => {
      ...
      if (!React || typeof React.createElement !== 'function') {
        throw new TypeError('dsh-fleet-cleaner [client]: React with createElement is required by ModuleLoader factory.');
      }

      const { FleetCleanerTab, FleetCleanerHud } = createModule(React);

      function applyClient(ctx) {
        return apply(ctx, { React });
      }
  ...
  // Browser auto-registration into DSH ModuleLoader
  if (typeof window !== 'undefined' && window.__ModuleLoader__) {
    window.__ModuleLoader__.load({
      id: 'dsh-fleet-cleaner',
